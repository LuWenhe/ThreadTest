...
public class MyThread2 extends Thread {
     ...
     * ThreadA end time =1540622759392
     ...
     * 而线程 AAA 要睡眠 5s，因此，等不到线程 AAA 执行，就先执行线程 ThreadA 中 join 后的代码，在等
     * 3s 后，在执行线程 AAA sleep 后的代码
     *
     * AAA begin time =1540623452338
     * AAA end time = 1540623457339
     * ThreadA end time =1540623457339
     *
     * 如果执行 thread2.join(7000)，此时输出是上面一种，因为线程 ThreadA 被阻塞 7s，此时轮到线程 AAA
     * 执行，因为只睡眠 5s，因此在线程 ThreadA 阻塞的时候，线程 AAA 就已经执行完毕，等到线程 AAA 执行完，
     * 线程 ThreadA 才开始执行 join 后面的代码
     */
    public static void main(String[] args) throws InterruptedException {
        MyThread2 thread2 = new MyThread2();
        thread2.setName("AAA");
        thread2.start();
        //调用 join 方法的线程 AAA 所在的线程 ThreadA 释放对象锁 thread2，线程 AAA
        //获得对象锁之后开始执行 run 方法
        thread2.join(6000);
